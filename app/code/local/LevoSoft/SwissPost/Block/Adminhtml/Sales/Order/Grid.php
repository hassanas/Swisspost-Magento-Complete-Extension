 ...
 */
/**
 * Class LevoSoft_SwissPost_Block_Adminhtml_Sales_Order_Grid
 */
class LevoSoft_SwissPost_Block_Adminhtml_Sales_Order_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    /**
    ...
    {
        parent::__construct();
        $this->setId('levosoft_swisspost_grid');
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->getSelect()->join(Mage::getConfig()->getTablePrefix() . 'sales_flat_order as sfo', 'sfo.entity_id=`main_table`.entity_id', array(
            ...
            'swisspost_label_exported' => 'swisspost_label_exported',
            'swisspost_label_exists' => 'swisspost_label_exists'
        ));
        $collection->addAttributeToFilter('shipping_method', array('like' => '%swisspost%'));
    ...
    {
        $helper = Mage::helper('levosoft_swisspost');
        ...
        /*$this->addColumn('shipping_method', array(
            ...
            'renderer' => 'LevoSoft_SwissPost_Block_Adminhtml_Sales_Order_Grid_Renderer_Shippingmethod'
        ));*/
        $this->addColumn('swisspost_label_exists', array(
            'header' => $helper->__('Swisspost Label'),
            'index' => 'swisspost_label_exists',
            'type' => 'options',
            'width' => '100px',
            'options' => array(
                0 => 'No',
        ...
        $this->addColumn('swisspost_label_exported', array(
            'header' => $helper->__('Label Downloaded'),
            'index' => 'swisspost_label_exported',
            'type' => 'options',
            'width' => '100px',
            'options' => array(
                0 => 'No',
        ...
        $this->addExportType('*/*/exportSwissPostOrdersCsv', $helper->__('CSV'));
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('generateAndComplete', array(
            'label' => Mage::helper('levosoft_swisspost')->__('Generate Label and Complete'),
            'url' => $this->getUrl('*/*/generateAndComplete'),
            'confirm' => Mage::helper('levosoft_swisspost')->__('Generating a label can take up to 1 second per label, please be patient during this process. It can take up to maximum 2 minutes. Do you want to continue?')
        ));
        $this->getMassactionBlock()->addItem('dowloadAllUndownloaded', array(
            'label' => Mage::helper('levosoft_swisspost')->__('Download all undownloaded'),
            'url' => $this->getUrl('*/*/dowloadAllUndownloaded'),
            'confirm' => Mage::helper('levosoft_swisspost')->__('Please refresh the page after downloading to review the confirmation messages including any problems encountered. Continue?')
        ));
        return $this;
    }
